...
os.system("mkdir ScrapedPDFEngg")
            ...
            for i in range(len(links)):
                ...
                if re.search(r'\.pdf', link.get('href', '')):
                    ...
                    try:
                        ...
                        target_text2 = "[IR-E-"
                        if check_text_in_pdf_url(pdf_url1, target_text) and check_text_in_pdf_url(pdf_url1, target_text2):
                            os.system("mkdir ScrapedPDFEngg/"+'"'+college+'"')
                            response = requests.get(pdf_url1)
                            if response.status_code == 200:
                                # Save the PDF to a local file
                                with open("ScrapedPDFEngg/"+college+"/"+str(i)+".pdf", "wb") as pdf_file:
                                    pdf_file.write(response.content)

                                with open("ScrapedPDFEngg/"+college+"/"+str(i)+".txt", "w") as file:
...
url = "https://www.nirfindia.org/2023/EngineeringRankingALL.html"
...
for i in range(len(df)):
    ...
    query = str(college) +" nirf engineering 2023 filetype:pdf"
    # print(college)
